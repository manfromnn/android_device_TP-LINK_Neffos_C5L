...
//modify by xingbin for shutdown device immediately when usb plug out 
#define UNPLUGGED_SHUTDOWN_TIME 0 //(1 * MSEC_PER_SEC)

#define BATTERY_FULL_THRESH     100 /* Modify by lichuangchuang for battery shutdown soc full (8909) SW00142738 20150525 */
...
#define LCD_TEST_FLAGS_PATH   "/sys/class/leds/lcd-test/flags"

//yuquan added begin
#define  BATTERY_MAX_FARME  5
#define  BATTERY_TEMP_OVERHEAT   (50*10)
#define  BATTERY_TEMP_OVERCOLD   (0*10)
//yuquan added end
...
/*yuquan added begin*/
    gr_surface surf_overheat;
    gr_surface surf_cold;
/*yuquan added end*/
};
/*modify by xingbin for charging display at power off begin*/
static struct frame batt_anim_frames[] = {
    {
	 .name = "charger/battery_0",//yuquan added
        .disp_time = 300,
        .min_capacity = 0,
        .level_only = false,
        .surface = NULL,
    ...
    {
	 .name = "charger/battery_1",//yuquan added	
        .disp_time = 400,
        .min_capacity = 20,
        .level_only = false,
        .surface = NULL,
    ...
    {
	 .name = "charger/battery_2",//yuquan added	
        .disp_time = 500,
        .min_capacity = 40,
        .level_only = false,
        .surface = NULL,
    ...
    {
	 .name = "charger/battery_3",//yuquan added	
        .disp_time = 600,
        .min_capacity = 60,
        .level_only = false,
        .surface = NULL,
    ...
    {
	 .name = "charger/battery_4",//yuquan added	
        .disp_time = 700,
        .min_capacity = 80,
        .level_only = false,         //SW00176850 modify by lihuiqin
        .surface = NULL,
    },
    {
	 .name = "charger/battery_5",//yuquan added	
        .disp_time = 750,
...
/*modify by xingbin for charging display at power off end*/
...
//yuquan added begin
static int set_backlight_off(void)
{
    int fd;
    char buffer[10];
    ...
    if (access(BACKLIGHT_PATH, R_OK | W_OK) != 0)
    {
        LOGE("Backlight control not support\n");
        return 0;
    }
...
//yuquan added begin
static void draw_overheat(struct charger *charger)
{
    int y;
    if (charger->surf_overheat) {
        draw_surface_centered(charger, charger->surf_overheat);
    } else {
        android_green();
        y = draw_text("Charging!", -1, -1);
        draw_text("?\?/100", -1, y + 25);
...
static void draw_cold(struct charger *charger)
{
    int y;
    if (charger->surf_cold) {
        draw_surface_centered(charger, charger->surf_cold);
    } else {
        android_green();
        y = draw_text("Charging!", -1, -1);
        draw_text("?\?/100", -1, y + 25);
...
static void redraw_screen(struct charger *charger)
{
    ...
    if (batt_anim->capacity < 0 || batt_anim->num_frames-2 == 0) {
        draw_unknown(charger);
    } else {
        draw_battery(charger);
        //draw_capacity(charger);//yuquan deleted
    }
    gr_flip();
}
...
//#endif//yuquan deleted end
    /* animation is over, blank screen and leave */
    if (batt_anim->cur_cycle == batt_anim->num_cycles) {
        reset_animation(batt_anim);
        charger->next_screen_transition = -1;
	 /*close backlight for display flicker by xingbin begin */
	 set_backlight_off();
	/*close backlight for display flicker by xingbin end */
        gr_fb_blank(true);
    ...
    if (batt_anim->cur_frame == 0) {
        ...
        if (batt_prop && batt_prop->batteryLevel >= 0 && batt_anim->num_frames-2 != 0) {
            int i;

            /* find first frame given current capacity */
            for (i = 1; i < batt_anim->num_frames-2; i++) {
...
/*don't enable display when no usb connected by xingbin*/
   ...
   if(charger->lcmtest_mode_enter){
	charger->batt_anim->cur_frame = 
	batt_anim->cur_frame = BATTERY_MAX_FARME + charger->lcmtest_pic_num;
	batt_anim->cur_cycle = 1;
   }
    ...
    if (charger->charger_connected) {
         ...
         */
        while (batt_anim->cur_frame < batt_anim->num_frames-2 &&
               batt_anim->frames[batt_anim->cur_frame].level_only)
            batt_anim->cur_frame++;
        if (batt_anim->cur_frame >= batt_anim->num_frames-2) {
            batt_anim->cur_cycle++;
...
/*enable/disable the lcd display at power down by xingbin start */
		if (key->pending) {
                if (!batt_anim->run) {
                    request_suspend(false);
...
/*enable/disable the lcd display at power down by xingbin end */
            }
        } else {
...
static void handle_lcd_test_state(struct charger *charger)
{
	
	 int ret = 1;
	 int  lcd_test_flags = 1;

	 ret = read_file_int(LCD_TEST_FLAGS_PATH, &lcd_test_flags);
	// LOGE("----------------------lcd_test_flags = %d -----------------------\n",lcd_test_flags);
	  if (!ret && (lcd_test_flags==1||lcd_test_flags==2)) {
	  	 charger->lcmtest_mode_enter = true;
		 charger->lcmtest_pic_num=lcd_test_flags;
	  }else{
		  charger->lcmtest_mode_enter = false;
		  charger->lcmtest_pic_num=0;
	  }
}
...
/*close lcd backlight when usb is unconnected by xingbin begin*/
    if(!charger->charger_connected){
	set_backlight_off();
    }
/*close lcd backlight when usb is unconnected by xingbin end*/	
    batt_prop = props;
}
...
/*yuquan added begin*/
    ret = res_create_display_surface("charger/battery_overheat", &charger->surf_overheat);
    if (ret < 0) {
        LOGE("Cannot load overheat image\n");
        charger->surf_overheat = NULL;
    }
    ret = res_create_display_surface("charger/battery_cold", &charger->surf_cold);
    if (ret < 0) {
        LOGE("Cannot load cold image\n");
        charger->surf_cold = NULL;
    }
/*yuquan added end*/
    charger->batt_anim = &battery_animation;

     #if 0//yuquan modified begin:
