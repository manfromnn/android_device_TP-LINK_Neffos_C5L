...
/* Added by yanwenlong for stop main server. (huawei) 2014-6-24 begin */
#define _REALLY_INCLUDE_SYS__SYSTEM_PROPERTIES_H_
#include <sys/_system_properties.h>
/* Added by yanwenlong for stop main server. (huawei) 2014-6-24 begin */
/* Added by yanwenlong for stop main server. (huawei) 2014-6-24 end */
...
static int lookup_keyword(const char *s)
{
    ...
    case 'e':
        ...
        if (!strcmp(s, "mergency")) return K_emergency;//add by yanwenlong for data_bk. 8909 20150310
        break;
    case 'g':
...
void stop_main_service(void)
{
    ...
    list_for_each(node, &service_list) {
        svc = node_to_item(node, struct service, slist);
        if (strstr(svc->classname, "main") != NULL)
        {
            re = kill(svc->pid, SIGKILL);
            ERROR("yep kill %s ret %d\n", svc->name, re);
        }
    }
}
/* Added by yanwenlong for stop main server. (huawei) 2014-6-24 end */
...
static void parse_line_service(struct parse_state *state, int nargs, char **args)
{
    ...
    case K_emergency: // add xingle
        svc->flags |= SVC_EMERGENCY;
        break;
