...
/* Added by yanwenlong for root check. (general) 2015-8-12 begin */
Value* root_checkFn(const char* name, State* state,
                         int argc, Expr* argv[]) 
{
    int result = 0;
    int fp = -1;
    ...
    if (access("/system/bin/su", 0) == 0 || access ("/system/xbin/su", 0) == 0)
        result = 1;
    if (result == 0)
    {
        fp = open("/dev/block/bootdevice/by-name/misc", O_RDWR);
        if (fp != -1)
        {
            lseek(fp, -512, SEEK_END);
            read(fp, info, 8);
            close(fp);    
        }
        if (info[0] != '\0' && strncmp(info, "unlocked", 8) == 0)
        {
            result = 1;
        }
    }
...
/* Added by yanwenlong for root check. (general) 2015-8-12 end */
...
void RegisterInstallFunctions() {
	...
	RegisterFunction("root_check", root_checkFn);
}
